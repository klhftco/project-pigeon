...
# import OWL-ViT detector instead of YOLO
...
FACE_LOST_COUNT = 10         # Frames without face detection before reverting to OWL-ViT
...
# --------------------
# Combined Drone‐Face‐Tracker with OWL-ViT
# --------------------
class DroneFaceTrackerOwl:
    def __init__(self, tello=None):
        self.tello = tello
        if not tello:
            # drone + vision
            self.tello = Tello()
        # Use OWL-ViT instead of YOLO
        self.detector = OwlDetector(model_path="google/owlvit-base-patch32")
        ...
        self.window_name = "OWL-ViT Face Tracker"
        ...
        # Lower confidence threshold for OWL-ViT (might need adjustment)
    ...
    def _draw_overlay(self, frame, confirmed_tracks, target_relative_pos=None):
        ...
        # Draw OWL-ViT indicator
        cv2.putText(frame, "OWL-ViT Detector", (width - 230, height - 50),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 165, 0), 2)  # Orange text
    ...
    def run(self, connected=False, target_texts=None):
        ...
        try:
            while True:
                ...
                if not self.face_mode:
                    # PERSON TRACKING MODE using OWL-ViT
                    # 1) OWL-ViT detect + track
                    detections = self.detector.detect(frame, conf_threshold=self.conf_threshold, target_texts=target_texts)
                ...
                # Display frame with overlay (already in BGR format)
                disp = img_to_draw
